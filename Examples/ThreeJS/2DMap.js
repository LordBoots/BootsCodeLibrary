...
mapCanvas.style.backgroundColor = 'rgba(0, 0, 0, 0.8)';
mapCanvas.style.display = 'none';
mapCanvas.style.zIndex = '1000';
const parentDiv = document.getElementById('renderDiv');
parentDiv.appendChild(mapCanvas);
// Map state
let isMapVisible = false;
const mapScale = 2; // Adjust this to change the map zoom level
...
// Add map toggle with 'M' key
window.addEventListener('keydown', (event) => {
    if (event.key.toLowerCase() === 'm') {
        isMapVisible = !isMapVisible;
        mapCanvas.style.display = isMapVisible ? 'block' : 'none';
    }
});
...
function renderMap() {
    ...
    for (let i = -gridCount; i <= gridCount; i++) {
        // Vertical lines
        const x = centerX + i * gridSize * mapScale;
        ctx.beginPath();
        ctx.moveTo(x, 0);
        ctx.lineTo(x, height);
        ctx.stroke();
        // Horizontal lines
        const y = centerY + i * gridSize * mapScale;
        ctx.beginPath();
        ctx.moveTo(0, y);
        ctx.lineTo(width, y);
        ctx.stroke();
    ...
    // Draw balls
    balls.forEach(ball => {
        const x = centerX + ball.body.position.x * mapScale;
        const y = centerY - ball.body.position.z * mapScale;

        ctx.beginPath();
        ctx.arc(x, y, 5, 0, Math.PI * 2);
        ctx.fillStyle = ball.mesh.material.color.getStyle();
        ctx.fill();
    });
    // Draw character if it exists
    if (characterBody) {
        const x = centerX + characterBody.position.x * mapScale;
        const y = centerY - characterBody.position.z * mapScale;
        ...
        const angle = character ? character.rotation.y : 0;
        ctx.beginPath();
        ctx.moveTo(x, y);
        ctx.lineTo(
            x + Math.sin(angle) * 20,
    ...
    const compassX = width - compassRadius - 20;
...
function animate() {
    ...
    composer.render();
    renderMap(); // Render the map overlay if visible
}
