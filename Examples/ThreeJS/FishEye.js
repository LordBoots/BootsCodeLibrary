...
// Create overlay text for fisheye controls
...
overlayText.innerHTML = 'Press UP/DOWN to change fisheye strength<br>Press LEFT/RIGHT to change fisheye radius';
parentDiv.appendChild(overlayText);
...
const fisheyeShader = {
    ...
    fragmentShader: `
        ...
        void main() {
            ...
            float vignetteAmount = smoothstep(radius, radius - 1.0, dist);
            
            vec2 distortedUv = vUv;
            float distortionStrength = 1.0 - vignetteAmount * strength;
            distortedUv = (distortedUv - 0.5) * distortionStrength + 0.5;
            
            vec4 color = texture2D(tDiffuse, distortedUv);
            color.rgb *= mix(1.0, vignetteAmount, strength);
            gl_FragColor = color;
        }
    `
};
...
fisheyePass.uniforms.radius.value = 2.5;
...
window.addEventListener('keydown', (event) => {
    ...
    if (event.code === 'ArrowUp') {
        fisheyePass.uniforms.strength.value = Math.min(1.0, fisheyePass.uniforms.strength.value + 0.1);
    }
    if (event.code === 'ArrowDown') {
        fisheyePass.uniforms.strength.value = Math.max(0.0, fisheyePass.uniforms.strength.value - 0.1);
    }
    if (event.code === 'ArrowRight') {
        fisheyePass.uniforms.radius.value = Math.min(5.0, fisheyePass.uniforms.radius.value + 0.2);
    }
    if (event.code === 'ArrowLeft') {
        fisheyePass.uniforms.radius.value = Math.max(1.0, fisheyePass.uniforms.radius.value - 0.2);
    }
});
...
function onWindowResize() {
    ...
    if (fisheyePass) {
        fisheyePass.uniforms.aspectRatio.value = width / height;
    }
}
