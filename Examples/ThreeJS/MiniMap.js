...
parentDiv.appendChild(minimapContainer);
...
const minimapConfig = {
    size: 200,
    scale: 0.5, // Scale factor for converting world coordinates to minimap coordinates
    playerColor: '#00ff00',
    ballColor: '#ff0000',
...
function updateMinimap() {
    ...
    ctx.fill();
    // Draw balls
    ctx.fillStyle = minimapConfig.ballColor;
    balls.forEach(ball => {
        const x = center + ball.body.position.x * minimapConfig.scale;
        const z = center + ball.body.position.z * minimapConfig.scale;
    ...
    // Draw player
    if (characterBody) {
        ctx.fillStyle = minimapConfig.playerColor;
        const playerX = center + characterBody.position.x * minimapConfig.scale;
        const playerZ = center + characterBody.position.z * minimapConfig.scale;

        // Draw player direction triangle
        ctx.save();
        ctx.translate(playerX, playerZ);
        ctx.rotate(character ? character.rotation.y : 0);
...
function onWindowResize() {
    ...
    const minimapSize = Math.min(width, height) * 0.2; // 20% of the smallest dimension
